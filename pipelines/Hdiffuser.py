...
def pipeline(args):
    ...
    high_nn_diffusion_planner = DiT1d(
    ...
    low_nn_diffusion_planner = DiT1d(
            obs_dim, emb_dim=args.planner_emb_dim,
            d_model=args.planner_d_model, n_heads=args.planner_d_model//64, depth=args.planner_depth, timestep_emb_type="fourier")
    ...
    print("High-Res Diffusion Planner:")
    report_parameters(high_nn_diffusion_planner)
    print("Low-Res Diffusion Planner:")
    report_parameters(low_nn_diffusion_planner)
    print(f"==============================================================================")

    # --------------- Classifier Free Guidance --------------------
    nn_condition_planner = MLPCondition(
        in_dim=obs_dim, out_dim=args.planner_emb_dim,
        hidden_dims=[args.planner_emb_dim, ],
        act=nn.SiLU(), dropout=args.planner_condition_dropout)

    # ----------------- Masking -------------------
    high_fix_mask = torch.zeros(((args.task.planner_horizon - 2) // args.task.planner_skip_steps + 2, obs_dim))
    high_fix_mask[0, :obs_dim] = 1.
    high_fix_mask[-1, :obs_dim] = 1.  # condition on goal state

    # --------------- Diffusion Model --------------------
    high_planner = ContinuousDiffusionSDE(
        high_nn_diffusion_planner, nn_condition=None,
        fix_mask=high_fix_mask, loss_weight=None, classifier=None, ema_rate=args.planner_ema_rate,
        device=args.device, predict_noise=args.planner_predict_noise, noise_schedule="linear")
    
    low_planner = ContinuousDiffusionSDE(
        low_nn_diffusion_planner, nn_condition=nn_condition_planner,
        fix_mask=None, loss_weight=None, classifier=None, ema_rate=args.planner_ema_rate,
        device=args.device, predict_noise=args.planner_predict_noise, noise_schedule="linear")
    ...
    if args.mode == "train":
        ...
        high_planner_lr_scheduler = CosineAnnealingLR(high_planner.optimizer, args.planner_diffusion_gradient_steps)
        high_planner.train()
        low_planner_lr_scheduler = CosineAnnealingLR(low_planner.optimizer, args.planner_diffusion_gradient_steps)
        low_planner.train()
        # Policy
        if args.use_diffusion_invdyn:
        ...
        for planner_batch, policy_batch in loop_two_dataloaders(planner_dataloader, policy_dataloader):
            ...
            high_planner_horizon_data = torch.zeros((planner_horizon_obs.shape[0],
                                                 (args.task.planner_horizon - 2) // args.task.planner_skip_steps + 2,
                                                 obs_dim), device=args.device)
            high_planner_horizon_data[:, args.task.planner_horizon // args.task.planner_skip_steps, :] = planner_horizon_obs[:, ::args.task.planner_skip_steps, :]
            high_planner_horizon_data[:, -1, :] = planner_horizon_obs[:, -1, :]

            # TODO low level planner horizon data
            B = planner_horizon_obs.shape[0]
            L = args.task.low_horizon
            T = args.task.planner_horizon

            low_planner_goal_idx = torch.randint(L - 1, T, (B,), device=args.device)
            rand_dist = torch.randint(args.task.low_goal_max_distance, (B,), device=args.device)
            low_planner_obs_start_idx = low_planner_goal_idx - rand_dist - L + 1
            low_planner_obs_start_idx = low_planner_obs_start_idx.clamp(0, T - L)  # 保证合法

            time_offsets = torch.arange(L, device=args.device).unsqueeze(0)       # [1, L]
            time_idx = low_planner_obs_start_idx.unsqueeze(1) + time_offsets      # [B, L]
            batch_idx = torch.arange(B, device=args.device).unsqueeze(1)          # [B, 1]

            low_planner_horizon_data = planner_horizon_obs[batch_idx, time_idx, :]  # [B, L, obs_dim]
            
            # TODO fix from here, we have batch data now
            if args.use_diffusion_invdyn:
